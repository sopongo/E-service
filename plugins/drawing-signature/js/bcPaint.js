	...
	**/
	var isDragged		= false,
		...
		templates 		= {
							...
							palette 		: $('<div class="col-sm-9 cols-md-9 bg-light rounded pt-4 text-center" id="bcPaint-palette"></div>'),
							color 			: $('<div class="bcPaint-palette-color"></div>'),
							canvasContainer : $('<div class="col-sm-9 col-md-9" id="bcPaint-canvas-container"></div>'),
							canvasPane 		: $('<canvas id="bcPaintCanvas" class="border border-dark rounded w-100 h-100"></canvas>'),
							bottom 			: $('<div class="col-sm-12 col-md-12 text-center mt-3" id="bcPaint-bottom"></div>'),
							buttonReset 	: $('<button type="button" class="btn btn-secondary btn-sm mr-1 align-top" id="bcPaint-reset"><i class="fas fa-eraser"></i> ล้าง</button>'),
	...
	**/
	$.fn.bcPaint = function (options) {
		...
		return this.each(function () {
			...
			//bottom.append(buttonReset); 	//ปุ่มเคลียร์
			//bottom.append(buttonSave); 	//ปุ่ม Export
			...
			$.each(colorSet.colors, function (i) {
				...
				palette.append(color).append(buttonReset);
    		});
	...
	$.extend(true, $.fn.bcPaint, {
		...
		*/
		export : function(){
			...
			windowOpen.document.write('<img id="img_signature" src="' + imgData + '" alt="Exported Image" />');
		},
