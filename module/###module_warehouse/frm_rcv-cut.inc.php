                        ...
                        <div class="row">
                          ...
                          <div class="col-sm-10 col-md-10 col-xs-10"><!--000-->
                              <div class="form-group">  
                                <label for="firstname">ไซต์งาน:</label> <div class="d-inline w-auto" id="offsupp_location"></div>
                              </div>
                          </div>
...
$(document).ready(function(){   
   $(document).on("click", ".btn-close", function (event) {
    ...
    $.ajax({
        ...
        success: function (row){
          ...
          $("#offsupp_location").html(row.location_name);
  ...
  $('form.addrcv').on('submit', function(event) {   
    var rcv_qty = $('#rcv_qty').val().replace(/,/g, '');
    var form_rcv = document.getElementById('needs-validation-rcv');  
    var date_rcv = new Date($('#date_rcv').val().replace(/\//g, '-'));
    var current_date= new Date('<?php echo date('Y-m-d');?>');
    if(date_rcv>current_date){
      swal("ผิดพลาด!", "วันที่รับเข้ามากกว่าวันที่ปัจจุบัน"); return false;      
    }
    if (form_rcv.checkValidity() === false) {  
      ...
      if(rcv_qty<=0){
        sweetAlert("ผิดพลาด !", "จำนวนต้องมากกว่า 0", "warning"); //The error will display
        $('body input[name="rcv_qty"]').closest("div").find('div[class^=invalid-feedback]').show(); 
        return false;
      }else{
        $('body input[name="rcv_qty"]').closest("div").find('div[class^=invalid-feedback]').hide();       
      }         
