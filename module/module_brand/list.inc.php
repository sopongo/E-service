<?PHP
/*echo "INSERT INTO `tb_site` (`id_brand`, `dept_initialname`, `brand_name`, `brand_status`) VALUES";
for($i=14;$i<=500;$i++){
  ...
  $('#some_button').click(function refreshData() {
    ...
    $('#example1').DataTable({
      ...
      ajax: {
        ...
        url: 'module/module_brand/datatable_processing.php',
        type: 'POST',
...
$(document).ready(function () {
  ...
  $('#example1_length').append('<div class="col-10 d-inline"><button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#modal-default" id="addData" data-backdrop="static" data-keyboard="false"><i class="fas fa-plus-circle"></i> เพิ่มแบรนด์</button></div>');
  $('input[type=search]').attr('placeholder', 'ชื่อแบรนด์');
  ...
  $(document).on('click','#addData',function(){   
    $('#exampleModalLabel span').html("เพิ่มแบรนด์");
  });

  
  $(document).on('click','.edit-data',function(){   
    $('#exampleModalLabel span').html("แก้ไขแบรนด์");
    ...
    $.ajax({
      ...
      success: function (data) {
        console.log(data);
        if(data){//id_brand, brand_name, brand_remark, brand_status
          $('#brand_name').val(data.brand_name);
          $('#id_row').val(data.id_brand);
          $('#exampleModalLabel span').html("แก้ไขแบรนด์: "+data.brand_name);
  ...
  $(document).on('click','.check-status',function(){
    ...
    /*module/module_brand/datatable_processing.php*/
</script>