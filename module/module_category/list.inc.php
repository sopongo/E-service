...
/*echo " INSERT INTO `tb_location` (`id_location`, `ref_id_site`, `ref_id_building`, `menu_code`, `location_name`, `location_status`) VALUES";
for($i=1;$i<=10;$i++){
...
foreach($cate as $x => $val) {
  echo "<br />(NULL, NULL, '1', NULL, NULL, NULL, '7', '".$val."', NULL, '2023-01-06 00:00:00', '1', NULL, NULL, '1'),";
}
*/
?>
...
$(document).ready(function () {
  ...
  $(document).on('click','.edit-data',function(){   
    ...
    $.ajax({
      ...
      success: function (data) {
        console.log(data);
        if(data){//id_menu, menu_code, level_menu, sort_menu, ref_id_menu, ref_id_sub, ref_id_dept, name_menu, desc_menu, menu_adddate, ref_id_user_add, menu_editdate, ref_id_user_edit, status_menu-->
          $('#menu_code').val(data.menu_code);
          $('#name_menu').val(data.name_menu);
          $('#id_row').val(data.id_menu);
          $('#ref_id_dept option:eq('+data.ref_id_dept+')').prop('selected', true);
          $('#desc_menu').val(data.desc_menu);
          $('#exampleModalLabel span').html("แก้ไขสถานที่ใช้งาน: "+data.name_menu);
          ...
          if(data.level_menu==1){
            $('.row-cate').addClass('d-none');
            $('#level_cate').prop('checked',true);
            $('#level_sub').prop('checked',false);
          }else{
            $('#level_cate').prop('checked',false);
            $('#level_sub').prop('checked',true);
  ...
  $(document).on('click','.check-status',function(){
    ...
    /*module/module_category/datatable_processing.php*/
</script>