...
<section class="content">
    ...
    <h6 class="display-8 d-inline-block font-weight-bold"><i class="fas fa-building"></i> <?PHP echo $title_act;?></h6>
...
<script type="text/javascript"> 
  ...
  $('#some_button').click(function refreshData() {
    ...
    $('#example1').DataTable({
      ...
      "aoColumnDefs": [
        { "bSortable": false, "aTargets": [0, 4, 5] }, //คอลัมน์ที่จะไม่ให้ฟังก์ชั่นเรียง
        { "bSearchable": false, "aTargets": [0, 4, 5] } //คอลัมน์ที่าจะไม่ให้เสริท
      ], 
      ajax: {
        beforeSend: function () {
          //จะให้ทำอะไรก่อนส่งค่าไปหรือไม่
        },
        url: 'module/module_building/datatable_processing.php',
        type: 'POST',
...
$(document).ready(function () {
  ...
  $('#example1_length').append('<div class="col-10 d-inline"><button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#modal-default" id="addData" data-backdrop="static" data-keyboard="false"><i class="fas fa-plus-circle"></i> เพิ่มอาคาร</button></div>');
  ...
  $(document).on('click','#addData',function(){   
    $('#exampleModalLabel span').html("เพิ่มอาคาร");
  });

  
  $(document).on('click','.edit-data',function(){   
    $('#exampleModalLabel span').html("แก้ไขอาคาร");
    ...
    $.ajax({
      ...
      success: function (data) {
        console.log(data);
        if(data){//tb_building  id_building, ref_id_site, building_initialname, building_name, building_status
          $('#building_initialname').val(data.building_initialname);
          $('#building_name').val(data.building_name);
          $('#id_row').val(data.id_building);
          $('#ref_id_site option:eq('+data.ref_id_site+')').prop('selected', true)
          $('#exampleModalLabel span').html("แก้ไขอาคาร: "+data.building_name);
          if(data.building_status==1){
  ...
  $(document).on('click','.check-status',function(){
    ...
    /*module/module_building/datatable_processing.php*/
</script>