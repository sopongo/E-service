...
$query_search = "";
if(!empty($search[0])){
    $query_search = " WHERE (tb_supplier.supplier_name LIKE '%".$search."%') AND tb_supplier.ref_id_site=".$_SESSION['sess_ref_id_site']." ";
}else{
    $query_search = " WHERE tb_supplier.ref_id_site=".$_SESSION['sess_ref_id_site']." ";
}
...
switch($_SESSION['sess_class_user']){
    ...
    break;
    default:
    $query_search.=' AND tb_supplier.ref_id_dept='.$_SESSION['sess_id_dept'].'';
    break;
}
...
$colunm_sort = array( //ใช้เรียงข้อมูล
    ...
    3=> "tb_supplier.supplier_name",
    4=> "tb_supplier.supplier_phone",    
    5=> "tb_dept.dept_initialname",        
    6=> "tb_supplier.supplier_remark",
    7=> "tb_supplier.supplier_status"
);
...
$fetchRow = $obj->fetchRows("SELECT tb_supplier.*, tb_dept.dept_name, tb_dept.dept_initialname, tb_site.site_initialname FROM tb_supplier LEFT JOIN tb_dept ON (tb_dept.id_dept=tb_supplier.ref_id_dept) LEFT JOIN tb_site ON (tb_site.id_site=tb_supplier.ref_id_site) ".$query_search." ORDER BY ".$orderBY." ".$_POST['order']['0']['dir']." LIMIT ".$_POST['start'].", ".$length." ");
