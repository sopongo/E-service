...
<section class="content">
    ...
    <h6 class="display-8 d-inline-block font-weight-bold"><i class="fas fa-map-marked-alt"></i> <?PHP echo $title_act;?></h6>
    ...
    <?php
      include_once 'module/module_site/frm_add-edit.inc.php'; //หน้า add/edit
    ?>
  ...
  $('#some_button').click(function refreshData() {
    ...
    $('#example1').DataTable({
      ...
      ajax: {
        ...
        url: 'module/module_site/datatable_processing.php',
        type: 'POST',
...
$(document).ready(function () {
  ...
  $('#example1_length').append('<div class="col-10 d-inline"><button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#modal-default" id="addData" data-backdrop="static" data-keyboard="false"><i class="fas fa-plus-circle"></i> เพิ่มไซต์งาน</button></div>');
  $('input[type=search]').attr('placeholder', 'ชื่อย่อ หรือ ชื่อไซต์งาน');
  ...
  $(document).on('click','#addData',function(){   
    $('#exampleModalLabel span').html("เพิ่มไซต์งาน");
  });

  
  $(document).on('click','.edit-data',function(){   
    $('#exampleModalLabel span').html("แก้ไขไซต์งาน");
    ...
    $.ajax({
      ...
      success: function (data) {
        console.log(data);
        if(data){//tb_site id_site, site_initialname, site_name, site_status
          $('#site_initialname').val(data.site_initialname);
          $('#site_name').val(data.site_name);
          $('#id_row').val(data.id_site);
          $('#exampleModalLabel span').html("แก้ไขไซต์งาน: "+data.site_name);
