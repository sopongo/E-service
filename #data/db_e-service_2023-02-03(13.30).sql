...
--
-- Host: localhost:3306
-- Generation Time: Feb 03, 2023 at 06:34 AM
...
(17, 'IT-AS-0005-0001', 'QWEE112343', '2023-01-27 00:00:00', 9, NULL, 407, 1, NULL, NULL, 'test add add test', '2023-01-27 15:48:02', 1, NULL, NULL, 2),
...
CREATE TABLE `tb_maintenance_request` (
  ...
  `status_approved` int NOT NULL COMMENT '1-อนุมัติ / 2-ไม่อนุมัติ(หน.ช่าง)',
  `ref_id_user_approver` int NOT NULL COMMENT 'REF.ไอดีผู้อนุมัติใบแจ้งซ่อม',
  `allotted_date` datetime NOT NULL COMMENT 'วันที่จ่ายงานให้ผู้ซ่อม',
  `allotted_accept_date` datetime NOT NULL COMMENT 'วันที่ผู้ซ่อมรับงาน',
  `related_to_safty` int NOT NULL COMMENT 'เกี่ยวข้องกับความปลอดภัย 	1-ไม่เกี่ยวข้อง/2-เกี่ยวข้อง',
  `problem_statement` longtext COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'อาการเสีย,ปัญหาที่พบ',
  `ref_id_job_type` int NOT NULL COMMENT 'ประเภทงานซ่อม',
  `urgent_type` int NOT NULL COMMENT '1-ด่วน/2-ไม่ด่วน',
  `outsource_service_status` int NOT NULL COMMENT '0, 1-ส่งซ่อมภายนอก/ จ้างช่างซ่อม',
  `caused_by_os` longtext COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'สาเหตุที่ส่งซ่อม 		เหตุผล:|:รายละเอียด / คั่นด้วย :|:',
  `ref_id_user_approve_os` int NOT NULL COMMENT 'ผู้อนุมัติส่งซ่อมภายนอก',
  `duration_serv_start` datetime NOT NULL COMMENT 'เวลาที่เริ่มซ่อม',
  `duration_serv_end` datetime NOT NULL COMMENT 'เวลาที่ซ่อมเสร็จ',
  `estimate_hand_over_date` datetime NOT NULL COMMENT 'วันที่คาดว่าจะเสร็จส่งมอบ',
  `hand_over_date` datetime NOT NULL COMMENT 'วันที่ส่งมอบงาน',
  `ref_id_user_hand_over` int NOT NULL COMMENT 'REF.ไอดีผู้ส่งมอบงาน',
...
--
ALTER TABLE `tb_attachment`
  MODIFY `id_attachment` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=11;
...
--
ALTER TABLE `tb_machine_master`
  MODIFY `id_machine` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=13;
...
--
ALTER TABLE `tb_machine_site`
  MODIFY `id_machine_site` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=25;
